...
export function SystemMonitor({
  ...
  useEffect(() => {
    const emptyData = Array.from({ length: MAX_DATA_POINTS }).map((_, i) => ({
      time: new Date(
        Date.now() - (MAX_DATA_POINTS - i) * 1000,
      ).toLocaleTimeString([], {
        hour12: false,
        second: "2-digit",
        minute: "2-digit",
          ...
          updatedPrev[updatedPrev.length - 1] = {
            ...
            latencyMs: Math.round(
              (lastPoint.latencyMs * lastPoint.throughput +
                newPoint.latencyMs) /
                (lastPoint.throughput + 1),
            ),
            confidence:
              (lastPoint.confidence * lastPoint.throughput +
                newPoint.confidence) /
              (lastPoint.throughput + 1),
          };
          return updatedPrev;
  ...
  const metrics = useMemo(() => {
    ...
    return {
      avgLatency: Math.round(
        latest10.reduce((acc, curr) => acc + curr.latencyMs, 0) /
          latest10.length,
      ),
      peakThroughput: Math.max(...data.map((d) => d.throughput)),
      avgConfidence: (
        latest10.reduce((acc, curr) => acc + curr.confidence, 0) /
        latest10.length
      ).toFixed(1),
    };
  }, [data]);
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#0f172a]/95 border border-slate-700 p-3 rounded-lg shadow-2xl backdrop-blur-md">
          <p className="text-slate-400 text-xs mb-2 font-mono border-b border-slate-700 pb-1">
            {label}
          </p>
        ...
        <div className="bg-[#0a0f18] border border-slate-800 rounded-lg p-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-white font-mono">
              ...
              <span className="text-sm text-slate-500">j/s</span>
            </p>
          </div>
        </div>
        <div className="bg-[#0a0f18] border border-slate-800 rounded-lg p-3 flex items-center gap-3">
