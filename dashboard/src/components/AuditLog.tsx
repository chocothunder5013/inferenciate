      ...
      <div className="flex-grow overflow-auto custom-scrollbar relative">
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 bg-[#0f172a] z-10 shadow-md">
            <tr>
              <th className="p-3 text-xs font-mono text-slate-400 font-medium border-b border-slate-700">
                Time
              </th>
              <th className="p-3 text-xs font-mono text-slate-400 font-medium border-b border-slate-700">
                Job ID
                ...
                logs.map((log, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3">
                        <div
                          className={`flex items-center gap-1.5 px-2 py-1 rounded w-max ${isHighConf ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}
                        >
                          {isHighConf ? (
                      ...
                      <td className="p-3 text-slate-400 flex items-center gap-1">
                        <Zap
                          className={`w-3 h-3 ${log.latencyMs > 100 ? "text-amber-500" : "text-slate-500"}`}
                        />
                        {log.latencyMs}ms
                      </td>
                    </motion.tr>
